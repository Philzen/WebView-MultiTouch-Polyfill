...
/**
 * WebClient class to polyfill missing multitouches on WebViews on Android 2.2 -
 * 2.3 devices support for Android 2.1 and earlier is on the roadmap:
 * https://github.com/Philzen/WebView-MultiTouch-Polyfill/issues/25
 *
 * This class will not interfere with your webview at all when running on
 * android 3 (honeycomb) or higher
 *
...
public class WebClient extends WebViewClient {
    ...
    /**
     * a compressed version of the javascript that will be injected and then
     * called in order to generate our polyfill touches
     */
    ...
    /**
     * The number of touches already working out-of-the-box (we'll assume at
     * least one for all devices)
     */
    protected int maxNativeTouches = 1;

    protected boolean validAndroidVersion = (Build.VERSION.SDK_INT >= 8 && Build.VERSION.SDK_INT <= 10);
    ...
    /**
     * Maintains a reference to the webview object for coding convenience
     * reasons
     */
    private WebView view;

    /**
     * true = use TimerClass to queue and send events, false = send events
     * immediately
     */
    private boolean timerClassEnabled = false;

    /**
     * Framerate for updates (Default: 60 Frames per second) play with it to see
     * the difference ( =1 =5 etc.)
     */
...
//	android.util.Log.v("console", "pagefinished_" + url);
    ...
    /**
     * Returns the collected touches and clears the TouchBuffer (needed for
     * TouchUpdater)
     *
     * @author fastrde
     * @return
    ...
    private boolean updateMoveBuffer(WebView view, MotionEvent event) {
        ...
        if (actionCode == MotionEvent.ACTION_MOVE) {
            ...
            for (int i = 0; i < event.getPointerCount(); i++) {
                if (Math.abs((int) lastMotionEvent.getX(i) - (int) event.getX(i)) <= moveThreshold
     ...
     * @param pointerIndex The index of the pointer in the collection that we
     * want to extract
     */
    ...
    /**
     * @param polyfillAllTouches If set to TRUE, all touch events will be
     * stopped and replaced by polyfills
...
//			android.util.Log.d("debug-console", EventJSON);
                ...
                /**
                 * FALSE : let other handlers do their work (good if we want to
                 * test for already working touchevents) TRUE : stop propagating
                 * / bubbling event to other handlers (good if we don't want
    ...
    /**
     * This nested class provides getter and setter for the javascript Interface
     * to WebClient. Issue:
     * https://github.com/Philzen/WebView-MultiTouch-Polyfill/issues/1 Fell free
     * to implement wanted Bridge functionality.
     *
     * @author fastr
     *
     */
    class jsInterface {
        ...
        /**
         * return JSON String of the current configuration. You have to
         * JSON.parse it on javascript-side
         */
